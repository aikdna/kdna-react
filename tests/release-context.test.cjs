...
test('release dependency binding rejects ranges and lock drift', () => {
  const ranged = structuredClone(PACKAGE_JSON);
  ranged.dependencies['@aikdna/kdna-web-client'] = '^0.2.2';
...
test('publish workflow is release-only, immutable, and checks out the exact tag', () => {
  ...
  assert.match(WORKFLOW, /git fetch origin main --no-tags/u);
  ...
  assert.match(WORKFLOW, /npm run test:web-stack-integration/u);
  assert.match(WORKFLOW, /already exists; refusing to hide registry drift/u);
  assert.doesNotMatch(WORKFLOW, /already published; skipping/u);
});
