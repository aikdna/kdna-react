...
const retiredTokens = [
  ...
  ['applicability', 'actual'].join('_'),
  ['source', 'attribution'].join('_'),
  ['cluster', 'identity'].join('_'),
  ['trace', 'AnswerSummary'].join(''),
  ['0', '9', '0'].join('.'),
...
const records = trackedFiles()
  .filter((file) => file !== allowlistPath && isTextFile(file))
...
for (const record of records) {
  ...
  if (generation.test(text)) findings.push(`${record.surface}:${record.path}: generation-style label`);
