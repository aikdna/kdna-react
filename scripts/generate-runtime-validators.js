...
const EXPECTED_AGGREGATE = '75dbb19a436667c82be430b4338bfca3fd55ba75459c47a4e90ee4f9a284de67';
...
async function generate() {
  ...
  const standalone = standaloneCode(ajv, { validateJudgmentTrace: JUDGMENT_TRACE_ID });
...
if (mode === '--write') {
  ...
  if (!fs.existsSync(outputPath) || fs.readFileSync(outputPath, 'utf8') !== generated) {
    throw new Error('Committed browser validator is stale; run npm run validators:generate.');
  }
  console.log('Canonical browser validator matches its schema closure.');
}
